function res = solve_es_ess_misocp(topo, loads, params)
%SOLVE_ES_ESS_MISOCP  Joint ES + ESS MISOCP.
%
% Combines ES active curtailment (Stage 1) with ESS active time-shifting
% and reactive injection (Stage 3). Key research question: can ES reduce
% required ESS count vs ESS-alone (C8)?
%
% VARIABLES
...
%   E(i,t)     in [SOC_min*E_cap, E_cap]*z_b — state of charge
%   Q_b(i,t)   in [0, Q_b_max*z_b(i)]     — reactive injection (pu)
%   v,Pij,Qij,ell,sv                       — DistFlow variables
%
% DistFlow effective demand (combined ES + ESS):
%   Peff = P_CL + (1-c)*P_NCL + P_ch - P_dis
%   Qeff = Q_CL + (1-c)*Q_NCL - Q_b
%
% PARAMS FIELDS
%   .rho / .u_min        NCL fraction / min service level
%   .N_e_max / .N_b_max  ES / ESS budgets
%   .E_cap_pu            ESS energy capacity per unit (default 1.0)
%   .P_ch_max_pu         ESS max charge power (default 0.5)
%   .P_dis_max_pu        ESS max discharge power (default 0.5)
%   .Q_b_max_pu          ESS inverter reactive capacity (default 0.10)
%   .eta_ch / .eta_dis   charge/discharge efficiency (default 0.95)
%   .SOC_init / .SOC_min initial/min SOC fraction (default 0.50/0.10)
%   .candidate_buses_es  ES candidates (default all non-slack)
%   .candidate_buses_b   ESS candidates (default all non-slack)
...
% DistFlow: ES curtails P+Q; ESS shifts P and injects Q
for t = 1:T
    for j = 1:nb
        ...
        Qeff = Q_CL(j,t) + (1-c(j,t))*Q_NCL(j,t) - Q_b(j,t);
...
if res.solver_ok
    ...
    fprintf('  ES+ESS: N_e=%d N_b=%d | Vmin=%.4f (h%d,b%d) | Loss=%.5f | Curt=%.1f%% | Qb=%.4f | VoltOK=%d | t=%.1fs\n', ...
        numel(e_sel), numel(b_sel), Vmin_24h, wh, VminBus_t(wh), ...
        sum(loss_t), 100*mean_curt, sum(Q_b_val(:)), res.voltage_ok, solve_time);
else
