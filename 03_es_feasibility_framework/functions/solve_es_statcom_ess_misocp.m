function res = solve_es_statcom_ess_misocp(topo, loads, params)
%SOLVE_ES_STATCOM_ESS_MISOCP  Joint ES + STATCOM + ESS MISOCP.
%
% Combines all three resource types in a single MISOCP:
%   ES     — binary placement, active+reactive NCL curtailment
%   STATCOM — binary placement, reactive injection only
%   ESS    — binary placement, P time-shifting + reactive injection
%
% DistFlow effective demand:
%   Peff = P_CL + (1-c)*P_NCL + P_ch - P_dis
%   Qeff = Q_CL + (1-c)*Q_NCL - Q_s - Q_b
%
% VARIABLES
...
%   Q_s(i,t)   in [0, Qs_max*z_s(i)]      reactive injection (pu)
...
%   .candidate_buses_es / .candidate_buses_s / .candidate_buses_b
...
% DistFlow: all three resources combined
for t = 1:T
    for j = 1:nb
        ...
        Qeff = Q_CL(j,t) + (1-c(j,t))*Q_NCL(j,t) - Q_s(j,t) - Q_b(j,t);
...
if strcmp(mode, 'feasibility')
    ...
    Obj = w_loss*lossCost + w_e*sum(z_e) + w_s*sum(z_s) + w_b*sum(z_b) ...
          + w_curt*curtCost + w_vio*svCost;
end
...
res.N_e_max = N_e_max; res.N_s_max = N_s_max; res.N_b_max = N_b_max;
...
if res.solver_ok
    ...
    fprintf('  Joint: N_e=%d N_s=%d N_b=%d | Vmin=%.4f (h%d,b%d) | Loss=%.5f | Curt=%.1f%% | Qs=%.4f Qb=%.4f | VoltOK=%d | t=%.1fs\n', ...
        numel(e_sel), numel(s_sel), numel(b_sel), Vmin_24h, wh, VminBus_t(wh), ...
        sum(loss_t), 100*mean_curt, sum(Q_s_val(:)), sum(Q_b_val(:)), res.voltage_ok, solve_time);
else
