function res = solve_es1_statcom_ess_misocp(topo, loads, params)
%SOLVE_ES1_STATCOM_ESS_MISOCP  Joint ES-1 (Hou) + STATCOM + ESS MISOCP.
%
% Extends solve_es_statcom_ess_misocp: ES devices use the Hou ES-1 model
% with independent reactive injection Q_es in addition to active curtailment.
% STATCOM and ESS provide supplementary reactive and storage support.
%
% DistFlow effective demand:
%   Peff = P_CL + (1-c)*P_NCL + P_ch - P_dis
%   Qeff = Q_CL + (1-c)*Q_NCL - Q_es - Q_s - Q_b
%
% VARIABLES
...
% DistFlow: ES-1 (P+Q curtailment + Q_es) + STATCOM (Q_s) + ESS (P shift + Q_b)
for t = 1:T
    for j = 1:nb
        ...
        Qeff = Q_CL(j,t) + (1-c(j,t))*Q_NCL(j,t) - Q_es(j,t) - Q_s(j,t) - Q_b(j,t);
...
if res.solver_ok
    ...
    fprintf('  ES1+Joint: N_e=%d N_s=%d N_b=%d | Vmin=%.4f (h%d,b%d) | Loss=%.5f | Curt=%.1f%% | Qes=%.4f Qs=%.4f Qb=%.4f | VoltOK=%d | t=%.1fs\n', ...
        numel(e_sel), numel(s_sel), numel(b_sel), Vmin_24h, wh, VminBus_t(wh), ...
        sum(loss_t), 100*mean_curt, sum(Q_es_val(:)), sum(Q_s_val(:)), sum(Q_b_val(:)), ...
        res.voltage_ok, solve_time);
else
    ...
    res.total_Qb      = NaN; res.net_dis   = NaN;
    res.total_sv      = NaN; res.max_sv    = NaN;
    res.voltage_ok    = false;
    fprintf('  ES1+Joint: INFEASIBLE | Code=%d | t=%.1fs\n', sol.problem, solve_time);
end
end
